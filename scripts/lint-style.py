#!/usr/bin/env python3
"""
Lint a file or files from CompPoly for style.

Sample usage:

    $ ./scripts/lint-style.py $(find CompPoly -name '*.lean')
...
a relative path (like ``CompPoly/Univariate/Basic.lean``) will produce errors
that contain the relative path, whilst linting absolute paths (like
``/path/to/CompPoly/Univariate/Basic.lean``) will produce errors with the
absolute path.
...
def format_errors(errors):
    ...
    for errno, line_nr, path in errors:
        ...
        if errno == ERR_TAC:
            output_message(path, line_nr, "ERR_TAC", "Files in CompPoly should not import the whole Mathlib.Tactic folder")
        if errno == ERR_TAC2:
            output_message(path, line_nr, "ERR_TAC2", "In the past, importing 'Lake' has led to dramatic slow-downs of the linter (see e.g. mathlib4#13779). Please consider carefully if this import is useful and make sure to benchmark it. If this is fine, feel free to allow this linter.")
        if errno == ERR_IBY:
