...
body, so proofs only ever see the verified ladder. Runtime agreement with
`native/comppoly_bt.c` is checked by `lake exe CompPolyBTExtTests`. Native inverses are
candidates: each caller verifies the result with one multiplication and falls back to the
verified inverse.
-/
...
/-- GF(2^64) multiplication; compiled code calls the native `comppoly_bt_mul64`. -/
...
/-- GF(2^64) inverse candidate; compiled code calls the native `comppoly_bt_inv64`.
Callers verify the result and fall back. -/
...
/-- `BT64` inversion: the native candidate checked by one multiplication, with the
verified inverse as fallback. -/
...
/-- GF(2^128) multiplication; compiled code calls the native `comppoly_bt128_mul`. -/
...
/-- GF(2^128) inverse candidate; compiled code calls the native `comppoly_bt128_inv`.
Callers verify the result and fall back. -/
@[extern "comppoly_bt128_inv"]
def inv128Candidate (v : @& FastBT128) : FastBT128 := FastBT128.inv v

/-- `inv128Candidate` agrees with the verified `FastBT128.inv`. -/
theorem inv128Candidate_eq (v : FastBT128) : inv128Candidate v = FastBT128.inv v := rfl

/-- GF(2^128) inversion: the native candidate checked by one multiplication, with the
verified inverse as fallback. -/
