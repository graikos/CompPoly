...
import CompPoly.Fields.Secp256k1.Fr.Fast.Prelude

/-!
# Fast Secp256k1.Fr Scalar Field — operations

A native-word implementation of secp256k1 scalar arithmetic as a sidecar to the canonical
`Secp256k1.ScalarField := ZMod SCALAR_FIELD_CARD` model. Fast values are stored as 256-bit
Montgomery residues below the prime (`x · 2²⁵⁶ mod p`, four `UInt64` limbs).
...
`CompPoly.Fields.Secp256k1.Fr.Fast.Prelude`. Because `ScalarField` is `Native256.FastField` at the
secp256k1 scalar-field tag, the generic algebraic instances resolve here automatically. This
module re-exports the named operations and `simp` lemmas at that instance.
-/
...
/-- Fermat exponent used for inversion in the Secp256k1.Fr scalar field. -/
def invExponent : Nat := Montgomery.Native256.invExponent (F := Secp256k1.ScalarField)
...
/-- Interpret a fast element in the canonical `Secp256k1.ScalarField` field. -/
@[inline]
def toField (x : ScalarField) : Secp256k1.ScalarField := Montgomery.Native256.toField x
...
def ofField (x : Secp256k1.ScalarField) : ScalarField :=
  Montgomery.Native256.ofField (F := Secp256k1.ScalarField) x

/-- Ring equivalence between the fast representation and the canonical `Secp256k1.ScalarField`. -/
def ringEquiv : ScalarField ≃+* Secp256k1.ScalarField :=
  Montgomery.Native256.ringEquiv (F := Secp256k1.ScalarField)

/-- Converting from the canonical field to fast form and back is the identity. -/
@[simp]
theorem toField_ofField (x : Secp256k1.ScalarField) : toField (ofField x) = x :=
  Montgomery.Native256.toField_ofField (F := Secp256k1.ScalarField) x
...
/-- The canonical-field interpretation distinguishes fast Secp256k1.Fr values. -/
theorem toField_injective : Function.Injective (toField : ScalarField → Secp256k1.ScalarField) :=
...
theorem toField_zero : toField (0 : ScalarField) = 0 :=
  Montgomery.Native256.toField_zero (F := Secp256k1.ScalarField)

/-- `toField` maps fast one to canonical one. -/
@[simp]
theorem toField_one : toField (1 : ScalarField) = 1 :=
  Montgomery.Native256.toField_one (F := Secp256k1.ScalarField)
...
@[simp]
theorem ringEquiv_symm_apply (x : Secp256k1.ScalarField) : ringEquiv.symm x = ofField x := rfl
...
@[simp]
theorem toField_natCast (n : Nat) : toField (n : ScalarField) = (n : Secp256k1.ScalarField) :=
  Montgomery.Native256.toField_natCast n

/-- Integer casts into fast form agree with integer casts into the canonical field. -/
@[simp]
theorem toField_intCast (n : Int) : toField (n : ScalarField) = (n : Secp256k1.ScalarField) :=
...
@[simp]
theorem toField_nnratCast (q : ℚ≥0) : toField (q : ScalarField) = (q : Secp256k1.ScalarField) :=
  Montgomery.Native256.toField_nnratCast q

/-- Rational casts into fast form agree with canonical-field casts. -/
@[simp]
theorem toField_ratCast (q : ℚ) : toField (q : ScalarField) = (q : Secp256k1.ScalarField) :=
