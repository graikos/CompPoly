...
A native-word implementation of BN254 scalar arithmetic as a sidecar to the canonical
`BN254.ScalarField := ZMod scalarFieldSize` model. Fast values are stored as 256-bit Montgomery
...
`CompPoly.Fields.BN254.Fast.Prelude`. Because `ScalarField` is `Native256.FastField` at the BN254
tag, the generic algebraic instances resolve here automatically. This module re-exports the named
operations and `simp` lemmas at the BN254 instance.
-/
...
/-- Interpret a fast element in the canonical `BN254.ScalarField` field. -/
@[inline]
def toField (x : ScalarField) : BN254.ScalarField := Montgomery.Native256.toField x
...
def ofField (x : BN254.ScalarField) : ScalarField :=
  Montgomery.Native256.ofField (F := BN254.ScalarField) x

/-- Ring equivalence between the fast representation and the canonical `BN254.ScalarField`. -/
def ringEquiv : ScalarField ≃+* BN254.ScalarField :=
  Montgomery.Native256.ringEquiv (F := BN254.ScalarField)

/-- Converting from the canonical field to fast form and back is the identity. -/
@[simp]
theorem toField_ofField (x : BN254.ScalarField) : toField (ofField x) = x :=
  Montgomery.Native256.toField_ofField (F := BN254.ScalarField) x
...
/-- The canonical-field interpretation distinguishes fast BN254 values. -/
theorem toField_injective : Function.Injective (toField : ScalarField → BN254.ScalarField) :=
...
theorem toField_zero : toField (0 : ScalarField) = 0 :=
  Montgomery.Native256.toField_zero (F := BN254.ScalarField)

/-- `toField` maps fast one to canonical one. -/
@[simp]
theorem toField_one : toField (1 : ScalarField) = 1 :=
  Montgomery.Native256.toField_one (F := BN254.ScalarField)
...
@[simp]
theorem ringEquiv_symm_apply (x : BN254.ScalarField) : ringEquiv.symm x = ofField x := rfl
...
@[simp]
theorem toField_natCast (n : Nat) : toField (n : ScalarField) = (n : BN254.ScalarField) :=
  Montgomery.Native256.toField_natCast n

/-- Integer casts into fast form agree with integer casts into the canonical field. -/
@[simp]
theorem toField_intCast (n : Int) : toField (n : ScalarField) = (n : BN254.ScalarField) :=
...
@[simp]
theorem toField_nnratCast (q : ℚ≥0) : toField (q : ScalarField) = (q : BN254.ScalarField) :=
  Montgomery.Native256.toField_nnratCast q

/-- Rational casts into fast form agree with canonical-field casts. -/
@[simp]
theorem toField_ratCast (q : ℚ) : toField (q : ScalarField) = (q : BN254.ScalarField) :=
