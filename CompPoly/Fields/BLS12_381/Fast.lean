...
/-!
# Fast BLS12_381 Scalar Field — operations

A native-word implementation of BLS12_381 scalar arithmetic as a sidecar to the canonical
`BLS12_381.ScalarField := ZMod scalarFieldSize` model. Fast values are stored as 256-bit Montgomery
...
`CompPoly.Fields.BLS12_381.Fast.Prelude`. Because `ScalarField` is `Native256.FastField` at the
BLS12-381 tag, the generic algebraic instances resolve here automatically. This module re-exports
the named operations and `simp` lemmas at the BLS12-381 instance.
-/
...
/-- Fermat exponent used for inversion in the BLS12_381 scalar field. -/
def invExponent : Nat := Montgomery.Native256.invExponent (F := BLS12_381.ScalarField)
...
/-- Interpret a fast element in the canonical `BLS12_381.ScalarField` field. -/
@[inline]
def toField (x : ScalarField) : BLS12_381.ScalarField := Montgomery.Native256.toField x
...
def ofField (x : BLS12_381.ScalarField) : ScalarField :=
  Montgomery.Native256.ofField (F := BLS12_381.ScalarField) x

/-- Ring equivalence between the fast representation and the canonical `BLS12_381.ScalarField`. -/
def ringEquiv : ScalarField ≃+* BLS12_381.ScalarField :=
  Montgomery.Native256.ringEquiv (F := BLS12_381.ScalarField)

/-- Converting from the canonical field to fast form and back is the identity. -/
@[simp]
theorem toField_ofField (x : BLS12_381.ScalarField) : toField (ofField x) = x :=
  Montgomery.Native256.toField_ofField (F := BLS12_381.ScalarField) x
...
/-- The canonical-field interpretation distinguishes fast BLS12_381 values. -/
theorem toField_injective : Function.Injective (toField : ScalarField → BLS12_381.ScalarField) :=
...
theorem toField_zero : toField (0 : ScalarField) = 0 :=
  Montgomery.Native256.toField_zero (F := BLS12_381.ScalarField)

/-- `toField` maps fast one to canonical one. -/
@[simp]
theorem toField_one : toField (1 : ScalarField) = 1 :=
  Montgomery.Native256.toField_one (F := BLS12_381.ScalarField)
...
@[simp]
theorem ringEquiv_symm_apply (x : BLS12_381.ScalarField) : ringEquiv.symm x = ofField x := rfl
...
@[simp]
theorem toField_natCast (n : Nat) : toField (n : ScalarField) = (n : BLS12_381.ScalarField) :=
  Montgomery.Native256.toField_natCast n

/-- Integer casts into fast form agree with integer casts into the canonical field. -/
@[simp]
theorem toField_intCast (n : Int) : toField (n : ScalarField) = (n : BLS12_381.ScalarField) :=
...
@[simp]
theorem toField_nnratCast (q : ℚ≥0) : toField (q : ScalarField) = (q : BLS12_381.ScalarField) :=
  Montgomery.Native256.toField_nnratCast q

/-- Rational casts into fast form agree with canonical-field casts. -/
@[simp]
theorem toField_ratCast (q : ℚ) : toField (q : ScalarField) = (q : BLS12_381.ScalarField) :=
